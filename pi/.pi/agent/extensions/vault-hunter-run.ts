...
function selectorArgs(input: {
  ...
  const values = [input.identity, input.id, input.name].filter(
    (value): value is string => typeof value === "string" && value.length > 0,
  );
  if (values.length !== 1)
    throw new Error(
      "atlas selector accepts exactly one of identity, id, or name.",
    );
...
function parseEnvelope(stdout: string): AtlasEnvelope {
  ...
  if (keys.join(",") !== "api_version,data,kind,meta")
    throw new Error("Atlas returned a malformed envelope.");
  if (envelope.api_version !== "atlas/v1" || typeof envelope.kind !== "string")
    throw new Error("Atlas returned a malformed envelope.");
...
function requireInitialDriverPlacement(
  request: Record<string, unknown>,
  placement: DriverPlacement,
): void {
  const initialDriver = ensureObject(
    request.initial_driver,
    "Atlas initial_driver",
  );
  const payload = ensureObject(
    initialDriver.payload,
    "Atlas initial_driver payload",
  );
  const participant = ensureObject(
    payload.registered_participant,
    "Atlas registered participant",
  );
  const session = ensureObject(
    participant.agent_session,
    "Atlas initial driver agent_session",
  );
  const herdr = ensureObject(participant.herdr, "Atlas initial driver herdr");
  if (
    session.source !== placement.agentSession.source ||
    session.kind !== placement.agentSession.kind ||
    session.value !== placement.agentSession.value
  ) {
    throw new Error(
      "Atlas initial_driver agent_session is not bound to this Pi session.",
    );
  }
  if (
    herdr.workspace_id !== placement.herdr.workspaceId ||
    herdr.tab_id !== placement.herdr.tabId ||
    herdr.pane_id !== placement.herdr.paneId ||
    herdr.terminal_id !== placement.herdr.terminalId
  ) {
    throw new Error(
      "Atlas initial_driver Herdr identity is not bound to this Pi session.",
    );
  }
}

async function driverPlacement(
  pi: ExtensionAPI,
  ctx: ExtensionContext,
  signal?: AbortSignal,
): Promise<DriverPlacement | undefined> {
  if (ctx.mode !== "tui") return undefined;
  const paneId =
    process.env.HERDR_ENV === "1" ? process.env.HERDR_PANE_ID : undefined;
  if (!paneId)
    throw new Error(
      "Interactive Vault Hunter drivers must run inside a Herdr pane.",
    );

  const response = await pi.exec("herdr", ["pane", "get", paneId], {
    signal,
    timeout: 5_000,
  });
  if (response.code !== 0)
    throw new Error(
      response.stderr.trim() || `Herdr could not resolve pane ${paneId}.`,
    );
  ...
  if (fields.some((value) => typeof value !== "string" || !value))
    throw new Error(
      `Herdr returned an incomplete identity for pane ${paneId}.`,
    );
  if (pane.pane_id !== paneId)
    throw new Error(`Herdr resolved ${paneId} as a different pane.`);
  if (
    !sessionFile ||
    session?.source !== "herdr:pi" ||
    session?.kind !== "path" ||
    session?.value !== sessionFile
  ) {
    throw new Error(`Herdr pane ${paneId} is not bound to this Pi session.`);
  }
  const paneCwd = pane.foreground_cwd ?? pane.cwd;
  if (!paneCwd || realpathSync(paneCwd) !== realpathSync(ctx.cwd))
    throw new Error(`Herdr pane ${paneId} is not in this Pi session's cwd.`);
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    description:
      "Validate the current Pi session, cwd, and complete Herdr tuple without invoking Atlas.",
  ...
  pi.registerTool({
    ...
    async execute(_id, params, signal) {
      ...
      if (params.identity || params.id || params.name)
        argv.push(...selectorArgs(params));
      if (params.run || params.pending) {
        if (params.resource !== "verifierattempts")
          throw new Error(
            "atlas_get only accepts run and pending for verifierattempts.",
          );
        if (typeof params.run === "string" && params.run.length > 0)
          argv.push("--run", params.run);
  ...
  pi.registerTool({
    ...
    label: "Atlas Create",
    description:
      "Create one Atlas resource through the public machine grammar.",
    ...
    async execute(_id, params, signal, _update, ctx) {
      ...
      if (Object.prototype.hasOwnProperty.call(request, "run")) {
        ...
        if (!placement)
          throw new Error(
            "Atlas Run creation requires an interactive Herdr-bound Pi session.",
          );
        requireInitialDriverPlacement(request, placement);
      }
      const details = await runAtlas(
        [params.resource as CreateResource, "create", "--json"],
        signal,
  ...
  pi.registerTool({
    ...
    description:
      "Read one Atlas Evidence envelope through the public machine grammar.",
    parameters: AtlasEvidenceGetSchema,
    async execute(_id, params, signal) {
      const details = await runAtlas(
        ["evidence", "get", ...selectorArgs(params)],
        signal,
      );
      return text(`Atlas returned ${details.kind}.`, details);
    },
  ...
  pi.registerTool({
    ...
    description:
      "Accept one exact pending verifier attempt through the public Atlas grammar.",
  ...
  pi.registerTool({
    ...
    description:
      "Reject one exact pending verifier attempt through the public Atlas grammar.",
  ...
  pi.registerTool({
    ...
    description:
      "After interactive confirmation, retire one exact Run through the public Atlas grammar.",
    parameters: AtlasRetireRunSchema,
    async execute(_id, params, signal, _update, ctx) {
      signal?.throwIfAborted();
      if (!ctx.hasUI)
        throw new Error(
          "Atlas Run retirement requires interactive confirmation; no UI is available.",
        );
      ...
      if (!confirmed)
        throw new Error(`Atlas Run ${target} retirement was declined.`);
      const details = await runAtlas(
        [
          "run",
          "retire",
...
export type AtlasAcceptVerifierAttemptInput = Static<
  typeof AtlasAcceptVerifierAttemptSchema
>;
export type AtlasRejectVerifierAttemptInput = Static<
  typeof AtlasRejectVerifierAttemptSchema
>;
