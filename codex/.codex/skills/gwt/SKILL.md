...
Use `gwt` instead of hand-writing `ghq get --bare` and `git worktree add` commands when setting up a new repository in the standard local layout.

## Command

`gwt` is available on `PATH` and wraps `$HOME/dotfiles/scripts/ghq-worktree-clone.sh`.
