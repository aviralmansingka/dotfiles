...
local function notify_fallback_once(bufnr, msg)
  if fallback_notified[bufnr] then
    return
  end
...
local function find_scope_node(bufnr, line, col)
  ...
  if not root or not parser then
    return nil, nil
  end
  ...
  for id, node in query:iter_captures(root, bufnr, 0, -1) do
    ...
    if cap == "function.outer" or cap == "class.outer" then
      ...
      local contains = (line > sr or (line == sr and col >= sc)) and (line < er or (line == er and col <= ec))
      if contains then
  ...
  if best_function then
    return best_function, "function"
  end
  if best_class then
    return best_class, "class"
  end
  return nil, nil
end
...
local function collect_identifiers_in_range(bufnr, start_line, end_line)
  ...
  local function walk(n)
    ...
    if IDENT_NODE_TYPES[n:type()] then
      if sr >= start_line and sr <= end_line then
        out[#out + 1] = n
      end
    end
    for child in n:iter_children() do
...
local function hover_text(result)
  ...
  if type(c) == "table" then
    if c.value then
      return c.value ~= "" and c.value or nil
    end
    local parts = {}
...
function M.render_prompt(question, ctx)
  ...
  parts[#parts + 1] = ""
  parts[#parts + 1] =
    "Keep the answer to 2-4 sentences at most. If prompted, provide a copyable markdown snippet containing the code the user asks for"
  return table.concat(parts, "\n")
end
...
---@param ctx { code: string, symbols: { name: string, hover: string }[] }
---@return string
function M.render_edit_prompt(question, ctx)
  local symbols_block = M.render_symbols_block(ctx.symbols)
  local parts = {
    "Edit the code to: " .. question,
    "",
    "Original code:",
    ctx.code,
  }
  ...
  parts[#parts + 1] = ""
  parts[#parts + 1] =
    "Reply with ONLY the modified version of the code. No prose, no explanations, no markdown fencing. Preserve all original indentation. Keep the change minimal and focused on the request."
  return table.concat(parts, "\n")
end
