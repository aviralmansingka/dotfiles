...
./vim-open.sh "$(pwd)" src/main.ts src/util.ts
```

The script handles:
- **Detecting an existing editor** in the current Herdr tab via `herdr
  pane list` + `herdr pane process-info`, looking for `vim`, `nvim`,
  or `vi`.
...
- **Editor selection**: prefers `vim`, then `nvim`, then `vi` — uses
  whichever is installed on the host.
...
local nvim config and launches the editor with `XDG_CONFIG_HOME` pointing
at it, so the editor uses the worktree's config instead of the global
`~/.config/nvim`.

Detection walks up the directory tree from cwd looking for
`nvim/.config/nvim/init.lua`. If found, `XDG_CONFIG_HOME` is set to
`<that-dir>/nvim/.config` for the launched editor process. This only
applies when `nvim` is the selected editor — `vim` and `vi` do not use
XDG_CONFIG_HOME.
...
and relaunch `/vim` to pick up the worktree config.
